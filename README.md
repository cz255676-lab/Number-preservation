...
* 🌐 **域名资产看板**：访问 `/domains` 管理域名、注册商、到期日、自动续费状态、续费费用和管理链接。
* 🛡️ **VPN 资产看板**：访问 `/vpns` 管理 VPN 服务、套餐、到期日、续费周期、自动续费状态和管理链接；请勿保存密码、私钥或订阅密钥。
* 🖥️ **云服务器资产看板**：访问 `/servers` 管理服务商、IP/主机名、地区/配置、到期日、续费周期、费用和管理链接；请勿保存 root 密码、SSH 私钥、API 密钥或控制面板令牌。
* ⏰ **智能 Telegram 提醒**：利用 CF Cron 定时任务，仅在 eSIM、域名、VPN 或云服务器命中各自到期规则时通过 TG 机器人发送告警。
